import{b as n}from"./bgParticles-BdBqC67m.js";function d(){const e=document.querySelector(".form");e.innerHTML="",e.insertAdjacentHTML("beforeend",`
  <div class="bg-roxo border-lilas border-4 w-[80%] lg:w-[55%] rounded-lg p-6 shadow-md mx-auto">
    ...
    <div id="sidebar" class="bg-preto text-branco border-l-2 border-branco/80 fixed top-0 right-0 z-50 hidden h-full w-64 translate-x-full transform shadow-xl transition-all duration-300 ease-in-out">
        ...
        >
        <img src="${i}" alt="sair" class="pixel h-8" />
        </button>
  ...
  `;e.insertAdjacentHTML("beforeend",A)}function g(){const e=document.querySelector(".search");JSON.parse(localStorage.getItem("desabafos")||"[]").length>0&&(e.classList.remove("hidden"),e.classList.add("flex"))}const u={Felicidade:{bg:"bg-amarelo",border:"border-amarelo",text:"text-amarelo"},Tristeza:{bg:"bg-azul",border:"border-azul",text:"text-azul"},Raiva:{bg:"bg-vermelho",border:"border-vermelho",text:"text-vermelho"},Ansiedade:{bg:"bg-laranja",border:"border-laranja",text:"text-laranja"},Motivação:{bg:"bg-rosa",border:"border-rosa",text:"text-rosa"},Tranquilidade:{bg:"bg-verde",border:"border-verde",text:"text-verde"},Medo:{bg:"bg-lilas",border:"border-lilas",text:"text-lilas"}},m=e=>{const{bg:A,border:t,text:o}=u[e.emocao];return`<div class="w-[85%] lg:w-[60%] p-4 gap-2">
        ...
        </div>`};function v(){const e=document.querySelector(".content");e.innerHTML="";const A=JSON.parse(localStorage.getItem("desabafos")||"[]");A.length>0?A.map(t=>e.insertAdjacentHTML("afterbegin",m(t))):e.insertAdjacentHTML("afterbegin",`<div class="text-center p-6">
      <p>Você pode fazer um desabafo agora!</p>  
    </div>`)}function b(){const e=document.getElementById("desabafoForm"),A=JSON.parse(localStorage.getItem("desabafos")||"[]");e.addEventListener("submit",t=>{t.preventDefault();const o=new Date,r=String(o.getDate()).padStart(2,"0"),a=String(o.getMonth()+1).padStart(2,"0"),s=o.getFullYear(),l={id:A.length>0?A[A.length-1].id+1:1,titulo:e.titulo.value,emocao:e.emocao.value,nivel:e.nivel.value,descricao:e.descricao.value,data:`${r}/${a}/${s}`};A.push(l),localStorage.setItem("desabafos",JSON.stringify(A)),e.reset(),location.reload()})}function f(){const e=document.getElementById("menu-btn"),A=document.getElementById("sidebar"),t=document.getElementById("close-btn");function o(){A.classList.remove("hidden"),setTimeout(()=>{A.classList.remove("translate-x-full"),A.classList.add("translate-x-0")},10)}function r(){A.classList.add("translate-x-full"),A.classList.remove("translate-x-0"),setTimeout(()=>{A.classList.add("hidden")},300)}e.addEventListener("click",a=>{a.stopPropagation(),o()}),t.addEventListener("click",r),document.addEventListener("click",a=>{!A.contains(a.target)&&!e.contains(a.target)&&r()})}g();document.addEventListener("DOMContentLoaded",()=>{n(),c(),f(),d(),v(),b()});
