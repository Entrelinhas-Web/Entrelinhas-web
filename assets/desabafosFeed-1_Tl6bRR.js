import{b as d}from"./bgParticles-BdBqC67m.js";function i(){const e=document.querySelector(".form");e.innerHTML="",e.insertAdjacentHTML("beforeend",`
  ...
  `;e.insertAdjacentHTML("beforeend",A)}function u(){const e=document.querySelector(".search");e.innerHTML="",JSON.parse(localStorage.getItem("desabafos")||"[]").length>0&&e.insertAdjacentHTML("beforeend",`
    <div class="searchBar w-[80%] lg:w-[55%] rounded-lg">
      <div
        class="flex items-center gap-3 bg-branco border-2 border-lilas rounded-3xl px-4 py-2"
      >
        <input
          type="text"
          name="titulo"
          placeholder="Buscar desabafos"
          class="text-preto placeholder:text-preto/70 outline-none w-full text-sm lg:text-md border-preto/60 border-2 p-2 rounded-2xl focus:border-lilas focus:ring-2 focus:ring-lilas"
          required
        />
        <img
          src="../../public/search-roxo.png"
          alt="Pesquisar"
          class="pixel h-5 w-5 lg:h-7 lg:w-7 cursor-pointer"
        />
      </div>
    </div>
  `)}const m={Felicidade:{bg:"bg-amarelo",border:"border-amarelo",text:"text-amarelo"},Tristeza:{bg:"bg-azul",border:"border-azul",text:"text-azul"},Raiva:{bg:"bg-vermelho",border:"border-vermelho",text:"text-vermelho"},Ansiedade:{bg:"bg-laranja",border:"border-laranja",text:"text-laranja"},Motivação:{bg:"bg-rosa",border:"border-rosa",text:"text-rosa"},Tranquilidade:{bg:"bg-verde",border:"border-verde",text:"text-verde"},Medo:{bg:"bg-lilas",border:"border-lilas",text:"text-lilas"}},b=e=>{const{bg:A,border:t,text:o}=m[e.emocao];return`<div class="w-[85%] lg:w-[60%] p-4 gap-2">
