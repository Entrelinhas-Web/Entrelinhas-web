import{b as m}from"./bgParticles-C_n2clo5.js";function v(){const e=document.querySelector(".form");e.innerHTML="",e.insertAdjacentHTML("beforeend",`
  <div class="bg-roxo border-lilas border-4 w-[80%] lg:w-[55%] rounded-lg p-6 shadow-md mx-auto">
    ...
    <div id="sidebar" class="bg-preto text-branco border-l-2 border-branco/80 fixed top-0 right-0 z-50 hidden h-full w-64 translate-x-full transform shadow-xl transition-all duration-300 ease-in-out">
        ...
        >
        <img src="${p}" alt="sair" class="pixel h-8" />
        </button>
  ...
  `;e.insertAdjacentHTML("beforeend",t)}function b(){const e=document.querySelector(".search");JSON.parse(localStorage.getItem("desabafos")||"[]").length>0&&(e.classList.remove("hidden"),e.classList.add("flex"));const a=e.querySelector("input");a&&a.addEventListener("input",()=>{const r=(a.value||"").trim().toLowerCase(),s=new CustomEvent("search-desabafo",{detail:{texto:r}});window.dispatchEvent(s)})}const c={Felicidade:{bg:"bg-amarelo",border:"border-amarelo",text:"text-amarelo"},Tristeza:{bg:"bg-azul",border:"border-azul",text:"text-azul"},Raiva:{bg:"bg-vermelho",border:"border-vermelho",text:"text-vermelho"},Ansiedade:{bg:"bg-laranja",border:"border-laranja",text:"text-laranja"},Motivação:{bg:"bg-rosa",border:"border-rosa",text:"text-rosa"},Tranquilidade:{bg:"bg-verde",border:"border-verde",text:"text-verde"},Medo:{bg:"bg-lilas",border:"border-lilas",text:"text-lilas"}},h=e=>{const{bg:t,border:a,text:r}=c[e.emocao];return`<div class="w-[85%] lg:w-[60%] p-4 gap-2">
            <a class="flex rounded-2xl border-2 ${a} overflow-hidden cursor-pointer hover:scale-105">
                ...
                <div class="content bg-preto flex flex-col flex-1 gap-3 p-2">
                    <div>
                        <h1 class="${r} font-bold">${e.titulo}</h1>
    ...
    >
        ...
        >
            ...
            <h1 class="${r} font-bold text-2xl mb-2">${t.titulo}</h1>
    ...
    `},T=e=>{const{border:t,text:a}=c[e.emocao],r=`modal-${e.id||Date.now()+Math.random()}`,s=x(r,e,t,a);document.body.insertAdjacentHTML("beforeend",s);const o=document.getElementById(r);if(!o)return;const A=o.querySelector(".js-close-popup"),l=o.querySelector(".js-edit-popup"),i=o.querySelector(".js-remove-popup"),g=o.querySelector(".bg-preto"),d=()=>{o.classList.add("hidden"),o.remove()};o.addEventListener("click",d),A&&A.addEventListener("click",d),g.addEventListener("click",u=>u.stopPropagation()),l&&l.addEventListener("click",()=>{console.log("Clicou em EDITAR o item:",e.id)}),i&&i.addEventListener("click",()=>{console.log("Clicou em REMOVER o item:",e.id)}),o.classList.remove("hidden")};function n(e=null){const t=document.querySelector(".content");if(!t)return;t.innerHTML="";const a=e??JSON.parse(localStorage.getItem("desabafos")||"[]");if(a.length===0){t.insertAdjacentHTML("afterbegin",`<div class="text-center p-6">
          <p>Você pode fazer um desabafo agora!</p>  
        </div>`);return}a.forEach(r=>{const s=h(r),o=document.createElement("div");o.innerHTML=s.trim();const A=o.firstElementChild;A&&(A.addEventListener("click",()=>{T(r)}),t.prepend(A))})}function E(){const e=document.getElementById("desabafoForm"),t=JSON.parse(localStorage.getItem("desabafos")||"[]");e.addEventListener("submit",a=>{a.preventDefault();const r=new Date,s=String(r.getDate()).padStart(2,"0"),o=String(r.getMonth()+1).padStart(2,"0"),A=r.getFullYear(),l={id:t.length>0?t[t.length-1].id+1:1,titulo:e.titulo.value,emocao:e.emocao.value,nivel:e.nivel.value,descricao:e.descricao.value,data:`${s}/${o}/${A}`};t.push(l),localStorage.setItem("desabafos",JSON.stringify(t)),e.reset(),location.reload()})}function w(){const e=document.getElementById("menu-btn"),t=document.getElementById("sidebar"),a=document.getElementById("close-btn");function r(){t.classList.remove("hidden"),setTimeout(()=>{t.classList.remove("translate-x-full"),t.classList.add("translate-x-0")},10)}function s(){t.classList.add("translate-x-full"),t.classList.remove("translate-x-0"),setTimeout(()=>{t.classList.add("hidden")},300)}e.addEventListener("click",o=>{o.stopPropagation(),r()}),a.addEventListener("click",s),document.addEventListener("click",o=>{!t.contains(o.target)&&!e.contains(o.target)&&s()})}b();document.addEventListener("DOMContentLoaded",()=>{m(),f(),w(),v(),n(),E()});window.addEventListener("search-desabafo",e=>{const t=(e?.detail?.texto||"").toLowerCase(),a=JSON.parse(localStorage.getItem("desabafos")||"[]");if(!t){n(a);return}const r=a.filter(s=>{const o=(s.titulo||"").toLowerCase(),A=(s.descricao||"").toLowerCase();return o.includes(t)||A.includes(t)});n(r)});
