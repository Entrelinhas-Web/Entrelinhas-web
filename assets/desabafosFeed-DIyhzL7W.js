import{b as m}from"./bgParticles-BtI45rmT.js";function u(){const e=document.querySelector(".form");e.innerHTML="",e.insertAdjacentHTML("beforeend",`
  <div class="bg-roxo border-lilas border-4 w-[80%] lg:w-[55%] rounded-lg p-6 shadow-md mx-auto">
    ...
    <div id="sidebar" class="bg-preto text-branco border-l-2 border-branco/80 fixed top-0 right-0 z-50 hidden h-full w-64 translate-x-full transform shadow-xl transition-all duration-300 ease-in-out">
        ...
        >
        <img src="${v}" alt="sair" class="pixel h-8" />
        </button>
  ...
  `;e.insertAdjacentHTML("beforeend",t)}function b(){const e=document.querySelector(".search");JSON.parse(localStorage.getItem("desabafos")||"[]").length>0&&(e.classList.remove("hidden"),e.classList.add("flex"))}const d={Felicidade:{bg:"bg-amarelo",border:"border-amarelo",text:"text-amarelo"},Tristeza:{bg:"bg-azul",border:"border-azul",text:"text-azul"},Raiva:{bg:"bg-vermelho",border:"border-vermelho",text:"text-vermelho"},Ansiedade:{bg:"bg-laranja",border:"border-laranja",text:"text-laranja"},Motivação:{bg:"bg-rosa",border:"border-rosa",text:"text-rosa"},Tranquilidade:{bg:"bg-verde",border:"border-verde",text:"text-verde"},Medo:{bg:"bg-lilas",border:"border-lilas",text:"text-lilas"}},h=e=>{const{bg:t,border:A,text:a}=d[e.emocao];return`<div class="w-[85%] lg:w-[60%] p-4 gap-2">
    ...
    `},x=e=>{const{border:t,text:A}=d[e.emocao],a=`modal-${e.id||Date.now()+Math.random()}`,r=f(a,e,t,A);document.body.insertAdjacentHTML("beforeend",r);const o=document.getElementById(a);if(!o)return;const s=o.querySelector(".js-close-popup"),l=o.querySelector(".js-edit-popup"),n=o.querySelector(".js-remove-popup"),c=o.querySelector(".bg-preto"),i=()=>{o.classList.add("hidden"),o.remove()};o.addEventListener("click",i),s&&s.addEventListener("click",i),c.addEventListener("click",g=>g.stopPropagation()),l&&l.addEventListener("click",()=>{console.log("Clicou em EDITAR o item:",e.id)}),n&&n.addEventListener("click",()=>{console.log("Clicou em REMOVER o item:",e.id)}),o.classList.remove("hidden")};function T(){const e=document.querySelector(".content");if(!e)return;e.innerHTML="";const t=JSON.parse(localStorage.getItem("desabafos")||"[]");if(t.length===0){e.insertAdjacentHTML("afterbegin",`<div class="text-center p-6">
          <p>Você pode fazer um desabafo agora!</p>  
        </div>`);return}t.forEach(A=>{const a=h(A),r=document.createElement("div");r.innerHTML=a.trim();const o=r.firstElementChild;o&&(o.addEventListener("click",()=>{x(A)}),e.prepend(o))})}function E(){const e=document.getElementById("desabafoForm"),t=JSON.parse(localStorage.getItem("desabafos")||"[]");e.addEventListener("submit",A=>{A.preventDefault();const a=new Date,r=String(a.getDate()).padStart(2,"0"),o=String(a.getMonth()+1).padStart(2,"0"),s=a.getFullYear(),l={id:t.length>0?t[t.length-1].id+1:1,titulo:e.titulo.value,emocao:e.emocao.value,nivel:e.nivel.value,descricao:e.descricao.value,data:`${r}/${o}/${s}`};t.push(l),localStorage.setItem("desabafos",JSON.stringify(t)),e.reset(),location.reload()})}function B(){const e=document.getElementById("menu-btn"),t=document.getElementById("sidebar"),A=document.getElementById("close-btn");function a(){t.classList.remove("hidden"),setTimeout(()=>{t.classList.remove("translate-x-full"),t.classList.add("translate-x-0")},10)}function r(){t.classList.add("translate-x-full"),t.classList.remove("translate-x-0"),setTimeout(()=>{t.classList.add("hidden")},300)}e.addEventListener("click",o=>{o.stopPropagation(),a()}),A.addEventListener("click",r),document.addEventListener("click",o=>{!t.contains(o.target)&&!e.contains(o.target)&&r()})}b();document.addEventListener("DOMContentLoaded",()=>{m(),p(),B(),u(),T(),E()});
